    ...
    $('.ui.form').form(
        {
            ...
            fields: {
                ...
                phone: {
                    identifier  : 'phone', rules: [{type   : 'empty', prompt : 'Введите номер телефона'}, {type   : 'maxLength[25]', prompt : 'Max 25 symbols'},
                        {type : 'regExp', value : '/^[+]*[(]{0,1}[0-9]{1,3}[)]{0,1}[-\\s\\./0-9]*$/g', prompt : 'not a phone'}],
                },
                header: { identifier  : 'header',  rules: [{type   : 'empty', prompt : 'Кто ищет новый дом?'}, {type   : 'maxLength[45]', prompt : 'So mach symbols, maximum 45!'}],
                },
                wallTextArea: { identifier  : 'wallTextArea',  rules: [{type   : 'empty', prompt : 'Опишите животное!'}, {type   : 'maxLength[1000]', prompt : 'So mach symbols, maximum 1000!'}],
                },
                inputFile: { identifier  : 'inputFile',  rules: [{type   : 'empty', prompt : 'Загрузите фотографии!'}],
                },
                animalsInput: { identifier  : 'animalsInput',  rules: [{type   : 'empty', prompt : 'Укажите категорию'}],
                },
                groupInput: { identifier  : 'groupInput',  rules: [{type   : 'empty', prompt : 'Укажите категорию'}],
                },
                breedsInput: { identifier  : 'breedsInput',  rules: [{type   : 'empty', prompt : 'Укажите категорию'}],
                },
                ageInput: { identifier  : 'ageInput',  rules: [{type   : 'empty', prompt : 'Укажите категорию'}],
                },
                genderInput: { identifier  : 'genderInput',  rules: [{type   : 'empty', prompt : 'Укажите категорию'}],
                }
